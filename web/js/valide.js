...
//			return /^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$/i.test(value);
	...
	price:{
		validator : function(value){
		return /^((\d{1,3})|(\d{1,3}\.\d{1}))$/.test(value);
	},
	message : '请输入价格，格式如999.9'
	},
	discount:{
		validator : function(value){
		var t=/^(\d{1,3})$/.test(value);
		if(!t) return false;
		return parseInt(t)<=100;
	},
	message : '请输入优惠折扣，格式如95,表示按原价的95%销售'
	},
	name:{
		validator : function(value){
		return /^[(\u0391-\uFFE5)|(A-Za-z)]{2,10}$/i.test(value);
	},
	message : "名称必须为2到10个汉字或字母"
	},
	
	productname:{
		validator : function(value){
		return /^[(\u0391-\uFFE5)|(A-Za-z)]{2,20}$/i.test(value);
	},
	message : "产品名称必须为2到20个汉字或字母"
	},
	...
	message : "密码必须为6到20位数字字母组成"
	},
	username:{
		validator : function(value){
		return /^\w{3,10}$/i.test(value);
	},
	message : "密码必须为3到10位字母数字下划线组成"
	}
});