 ...
 */
 	...
 	page_touch_end	: function(e){
 		...
 		if(Math.abs(car2._touchDeltaY)>10){
 			car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transition')] = 'http://img0.hx.com/magazine/js/all .3s';
 			car2._page.eq(car2._pageNow)[0].style[car2._prefixStyle('transition')] = 'http://img0.hx.com/magazine/js/all .3s';
 		}
 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','load.gif'/*tpa=http://img0.hx.com/magazine/img/load.gif*/);
			}else{
				// 把原来的图片预先保存下来
 ...
 */
	//插件启动函数
 	plugin : function(){
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams				: ["<img src='http://test.56jyt.com/reddaxi/images/audio_widget_01@2x.png'/>","<img src='http://test.56jyt.com/reddaxi/images/audio_widget_01@2x.png'/>"], 
			steamHeight			: 100,
			steamWidth			: 44 
		});
...
//			url = 'page_01_bg@2x.jpg'/*tpa=http://img0.hx.com/magazine/js/img/page_01_bg@2x.jpg*/,
